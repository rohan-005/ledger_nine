      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {RESEARCH_AREAS.map((area) => {
          ...
          for (const ev of agentEvidence) {
            ...
            const urls = sourceMap.get(ev.sourceType) || [];
            if (ev.sourceUrl && !urls.includes(ev.sourceUrl)) {
              urls.push(ev.sourceUrl);
            }
            sourceMap.set(ev.sourceType, urls);
          }

          const dataSourcesElements = Array.from(sourceMap.entries()).map(([sourceType, urls]) => {
            const name = getFriendlySourceName(sourceType);
            if (urls.length > 0) {
              return (
                <div key={sourceType} className="space-y-0.5">
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-0.5">
                    {urls.map((url, i) => (
                      <a
                        ...
                        className="text-[10px] text-blue-600 hover:underline font-mono bg-blue-50 px-1 py-0.5 rounded border border-blue-100"
                      >
                        Source URL {i + 1}
                      </a>
                    ))}
          ...
          return (
            <div key={area.id} className="bg-white border border-border rounded-xl p-5 shadow-xs flex flex-col justify-between space-y-4">
              ...
              <div className="space-y-2 text-xs border-t border-border/40 pt-3">
                <p className="text-[10px] text-foreground-secondary font-bold uppercase tracking-wider">Data Sources Consulted</p>
                {dataSourcesElements.length > 0 ? (
                  <div className="space-y-2 mt-1">
                    {dataSourcesElements}
                  </div>
                ) : (
                  <p className="text-foreground-muted italic">No specific sources consulted or unavailable.</p>
                )}
              </div>
            </div>
          );
