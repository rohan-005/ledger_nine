...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      const results = await fmpClient.search(q, 10);
      if (results && Array.isArray(results)) {
        providerMatches = results.map((item: any) => {
          const symbol = String(item.symbol || "").toUpperCase();
          const name = String(item.name || symbol);
          const rawExchange = String(item.exchange || item.exchangeShortName || "").toUpperCase();
